...
void calculate_revenue() {
    float cost_per_unit, selling_price_per_unit;
    int units_in_stock, units_sold;

        printf("Enter cost per unit: ");
        scanf("%f", &cost_per_unit);
        printf("Enter selling price per unit: ");
        scanf("%f", &selling_price_per_unit);
        printf("Enter units in stock: ");
        scanf("%d", &units_in_stock);
        printf("Enter units sold: ");
        scanf("%d", &units_sold);

    float total_revenue_earned = selling_price_per_unit * units_sold;
    float projected_total_revenue = selling_price_per_unit * (units_in_stock);
...
void calculate_gross_profit_loss() {
    ...
    float gross_profit_loss_per_unit = selling_price - cost;
    float gross_profit_loss_total = gross_profit_loss_per_unit * units_sold;
    float gross_profit_loss_percentage = (gross_profit_loss_total / (cost * units_sold)) * 100;
...
void calculate_operating_profit() {
    float total_revenue, total_gross_profit, total_expenses;

        printf("Enter total gross profit: ");
        scanf("%f", &total_gross_profit);
        printf("Enter total expenses: ");
        scanf("%f", &total_expenses);

    float operating_profit = total_gross_profit - total_expenses;
...
void calculate_tax_liabilities() {
    float total_gross_profit, tax_percentage;
    ...
    float total_tax_liability = (total_gross_profit * tax_percentage) / 100;
...
void calculate_net_profit() {
    float total_operating_profit, tax_percentage;

    printf("Enter total operating profit: ");
    scanf("%f", &total_operating_profit);
    printf("Enter tax percentage: ");
    scanf("%f", &tax_percentage);

    float total_net_profit = total_operating_profit * (1 - (tax_percentage / 100));
...
void calculate_discount() {
    ...
    float discounted_selling_price = selling_price * (1 - discount_percentage / 100);
    float original_gross_profit = (selling_price - cost) * total_units;
    float new_gross_profit = (discounted_selling_price - cost) * total_units;
    float original_gross_profit_percentage = (original_gross_profit / (cost * total_units)) * 100;
    float new_gross_profit_percentage = (new_gross_profit / (cost * total_units)) * 100;
...
void future_growth_analysis() {
    float cost, net_profit_percentage;
    int units_in_stock, units_sold;

        printf("Enter cost per unit: ");
        scanf("%f", &cost);
        printf("Enter net profit percentage: ");
        scanf("%f", &net_profit_percentage);
        printf("Enter units in stock: ");
        scanf("%d", &units_in_stock);
        printf("Enter units sold: ");
        scanf("%d", &units_sold);

    float net_profit_earned_so_far = (cost * net_profit_percentage / 100) * units_sold;
    float projected_total_net_profit = (cost * net_profit_percentage / 100) * (units_in_stock + units_sold);
...
void gross_profit_margin_analysis() {
    ...
    for (int i = 0; i < num_prices; i++) {
        ...
        float profit = selling_price - cost;
        float profit_percentage = (profit / cost) * 100;
